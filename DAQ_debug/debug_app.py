...
# Create fake device and streamer
...
# Run Quart server using asyncio.run (similar to main app)
async def run_quart_server():
    await quart_app.run_task(host='127.0.0.1', port=5001, debug=False)
...
if __name__ == '__main__':
    ...
    dash_thread = threading.Thread(target=lambda: app.run_server(debug=False, host='127.0.0.1', port=8050), daemon=True)
    dash_thread.start()
    
    # Small delay to ensure Dash server starts
    time.sleep(2)
    
    # Run Quart server in main thread (similar to main app pattern)
    try:
        asyncio.run(run_quart_server())
